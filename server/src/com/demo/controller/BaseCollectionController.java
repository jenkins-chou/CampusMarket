...
@CrossOrigin
public class BaseCollectionController  extends Controller {
	
	public static final String DB_TABLE = "base_collection";
	...
	@CrossOrigin
	public void getAll(){
		List<BaseCollectionModel> models = BaseCollectionModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void get(){
		...
		List<BaseCollectionModel> models = BaseCollectionModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BaseCollectionModel model = getModel(BaseCollectionModel.class, "", true);
	...
	public void search(){
		...
		List<BaseCollectionModel> models = BaseCollectionModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
