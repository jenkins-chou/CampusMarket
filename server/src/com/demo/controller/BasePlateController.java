...
@CrossOrigin
public class BasePlateController  extends Controller {
	
	public static final String DB_TABLE = "base_plate";
	...
	public void get(){
		...
		List<BasePlateModel> models = BasePlateModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BasePlateModel model = getModel(BasePlateModel.class, "", true);
	...
	public void search(){
		...
		List<BasePlateModel> models = BasePlateModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
