...
@CrossOrigin
public class BaseWishlistController  extends Controller {
	
	public static final String DB_TABLE = "base_wishlist";
	...
	public void getAllByProviderMobile(){
		...
		List<BaseWishlistModel> models = BaseWishlistModel.dao.find("select * from "+DB_TABLE+" where wish_provider = "+user_id+" and del != 'delete'");
	...
	public void getAllMobile(){
		List<BaseWishlistModel> models = BaseWishlistModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put("list", models);
	...
	public void get(){
		...
		List<BaseWishlistModel> models = BaseWishlistModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BaseWishlistModel model = getModel(BaseWishlistModel.class, "", true);
	...
	public void search(){
		...
		List<BaseWishlistModel> models = BaseWishlistModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
