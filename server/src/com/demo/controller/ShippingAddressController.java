...
@CrossOrigin
public class ShippingAddressController  extends Controller {
	
	public static final String DB_TABLE = "shipping_address";
	...
	public void setDefault(){
		...
		List<ShippingAddressModel> models = ShippingAddressModel.dao.find("select * from "+DB_TABLE+" where user_id = "+user_id+" and del != 'delete'");
		
		if(models != null && models.size()>0){
			for(ShippingAddressModel model : models){
				model.set("is_default", "false");
			}
		}
	
		ShippingAddressModel defaultModel = ShippingAddressModel.dao.findById(id);
		defaultModel.set("is_default", "true");
	...
	@CrossOrigin
	public void getAll(){
		List<ShippingAddressModel> models = ShippingAddressModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void get(){
		String id = getPara("id");
		List<ShippingAddressModel> models = ShippingAddressModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			ShippingAddressModel model = getModel(ShippingAddressModel.class, "", true);
	...
	public void search(){
		...
		List<ShippingAddressModel> models = ShippingAddressModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
